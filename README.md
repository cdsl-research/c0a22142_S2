...
Pythonをインストールするには，Python.org からインストーラーをダウンロードし実行する．

インストール画面が表示されたら，Add Python 〇.〇 to PATH のチェックボックスに必ずチェックを入れる．

「Install Now」をクリックしてインストールを開始する．

コマンドプロンプトまたはターミナルを開き，python --version または py --versionでバージョンを確認し，表示されれば成功である．
